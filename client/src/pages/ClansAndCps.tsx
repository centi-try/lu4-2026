...
export default function ClansAndCps() {
  ...
  const clansQ = trpc.clanModule.clans.list.useQuery();
  const cpsQ = trpc.clanModule.commandParties.list.useQuery();
  const unassignedQ = trpc.clanModule.commandParties.unassigned.useQuery(undefined, { enabled: isSuperAdmin });
  ...
  const invalidateAll = () => {
    utils.clanModule.commandParties.invalidate();
    utils.clanModule.clans.invalidate();
  };

  // Clan mutations
  const createClanMut = trpc.clanModule.clans.create.useMutation({
    onSuccess: () => { invalidateAll(); toast.success('Clan creado'); },
    onError: (e: any) => toast.error(e.message),
  });

  const createCp = trpc.clanModule.commandParties.create.useMutation({
    onSuccess: () => { invalidateAll(); toast.success('CP creada'); },
    onError: (e: any) => toast.error(e.message),
  });
  const updateCp = trpc.clanModule.commandParties.update.useMutation({
    onSuccess: () => { invalidateAll(); toast.success('CP actualizada'); },
    onError: (e: any) => toast.error(e.message),
  });
  const deleteCp = trpc.clanModule.commandParties.delete.useMutation({
    onSuccess: () => { invalidateAll(); toast.success('CP eliminada'); },
    onError: (e: any) => toast.error(e.message),
  });
  const setMemberStatus = trpc.clanModule.commandParties.setMemberStatus.useMutation({
    onSuccess: () => { invalidateAll(); toast.success('Estado actualizado'); },
    onError: (e: any) => toast.error(e.message),
  });
  const reassignMember = trpc.clanModule.commandParties.reassignMember.useMutation({
    onSuccess: () => { invalidateAll(); toast.success('Miembro reasignado'); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const handleCreateClan = () => {
    ...
    createClanMut.mutate({ name: newClanName.trim() });
    setNewClanName('');
  ...
  return (
    <AppShell>
      <div className="space-y-6 p-6">
          ...
          {isSuperAdmin && (
            <div className="flex gap-2">
              <button
                ...
                style={{ background: 'rgba(123,241,214,0.15)', border: '1px solid rgba(123,241,214,0.3)', color: '#7bf1d6' }}
              >
                <Plus className="h-5 w-5" /> Nuevo Clan
              </button>
              <button
        ...
        {/* Create Clan form */}
        {showCreateClan && isSuperAdmin && (
          <div className="rounded-xl p-4" style={{ background: 'rgba(123,241,214,0.06)', border: '1px solid rgba(123,241,214,0.2)' }}>
            ...
            <div className="flex gap-3">
              ...
              />
              <button onClick={handleCreateClan} disabled={!newClanName.trim()}
                className="rounded-lg px-4 py-2 text-sm font-medium"
                style={{ background: 'rgba(123,241,214,0.2)', color: '#7bf1d6', opacity: !newClanName.trim() ? 0.4 : 1 }}>
                Crear
              </button>
              <button onClick={() => setShowCreateClan(false)} className="rounded-lg px-3 py-2"
              ...
              {unassigned.map((u: any) => (
                <div key={u.id} className="flex items-center justify-between p-3 px-4">
                  ...
                  <button
                    onClick={() => {
                      setReassignModal(u);
                      setReassignClanId(u.clanId ? String(u.clanId) : '');
                      setReassignCpId('');
                    }}
...
function EditCpModal({
  ...
  const clanMembersQ = trpc.clanModule.commandParties.clanMembers.useQuery(
...
function CpRow({
  cp, expanded, onToggle, isSuperAdmin, currentUserId,
  ...
  const membersQ = trpc.clanModule.commandParties.members.useQuery(
    { cpId: Number(cp.id) },
    { enabled: expanded },
  );
  const members = (membersQ.data || []) as any[];
  const isLeader = Number(cp.leaderId) === currentUserId;
  const canManageMembers = isSuperAdmin || isLeader;
...
function MemberRow({
  ...
  const addSecChar = trpc.clanModule.commandParties.addSecondaryChar.useMutation({
    onSuccess: () => {
      utils.clanModule.commandParties.invalidate();
  ...
  const updateSecChar = trpc.clanModule.commandParties.updateSecondaryChar.useMutation({
    onSuccess: () => {
      utils.clanModule.commandParties.invalidate();
  ...
  const deleteSecChar = trpc.clanModule.commandParties.deleteSecondaryChar.useMutation({
    onSuccess: () => {
      utils.clanModule.commandParties.invalidate();
  ...
  const classesQ = trpc.clanModule.commandParties.listClasses.useQuery(undefined, { enabled: expanded && canEditAlts });
